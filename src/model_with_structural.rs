...
//use crate::structural_model::StructuralModel;
...
pub (crate)struct Model{
    ...
    gbcs: Vec<[bool;3]>,
    //pub (crate) structural_model: StructuralModel,
    //temp_change:Vec<f64>
}
...
impl Model{
    ...
    pub fn new(nodes:Vec<Node>,activation_cells_length:usize, sp_heat_cap_interp: Interpolator, beadwidth:f64, beadheight:f64,
               turn_off_layers_at:usize, stm: StructuralModel, gbcs: Vec<[bool;3]>)->Model{
        return Model{
            nodelist: nodes,
            ...
            //temp_change:Vec::with_capacity(activation_cells_length);
        }
    }
                   ...
                   orientation:[f64;2],layer:usize,cell_indices:&mut Vec<usize>)
    {
        ...
        self.stm.add_element(&self.nodelist,node_no.clone(),& [false;24]);
        ...
        self.templist_old.push(init_temp.clone());
    ...
    pub fn run_model(&mut self, time:f64, dt:f64, conv_coeff:f64, t_env:f64, file:&mut BufWriter<File>,
                     ...
                     maxthreads:usize, cell_indices:&Vec<usize>, datastorer:&mut (Vec<f64>,Vec<Vec<f64>>,Vec<usize>) ) {
        ...
        self.templist_old[new_cell_slice_start..new_cell_slice_end+1].copy_from_slice(active_temps);
        ...
        let mut tempchange = Vec::with_capacity(self.templist.len());
        for i in 0..self.templist.len(){
            tempchange.push(self.templist[i] - self.templist_old[i]);
        }
        let loads = self.stm.generate_load_vec_bc(tempchange,8e-4,8e-4,4e-4,1e-6);
        self.stm.structural_analysis_solve(&loads);
    ...
    pub fn update_model(mu:&mut ModelUpdater,mdl:&mut Model,bw:&mut BufWriter<File>,areas_and_dists:[f64;5],time_step:f64,
                        maxthreads:usize, input_data:([f64;3],f64,f64,f64,[f64;2]), pool: &ThreadPool)->(Vec<f64>,Vec<Vec<f64>>,Vec<usize>)
    {
        ...
        let mut datastorer = (store_global_time, store_temp_data, store_starting_cell);
        ...
        let mut utemp = Vec::with_capacity(mu.activation_times.len());
        ...
        println!("cell 1 added");
        utemp.push([0.0 as f64,0.0 as f64,0.0 as f64]);
        ...
        for i in 1..endval-1 {
            ...
            let time = next_cell_info2.0 - next_cell_info.0;
            // global_time = global_time + time;
            if i%10 == 0 {println!("{} out of {}, timeperiod {}",i, endval, time);}
